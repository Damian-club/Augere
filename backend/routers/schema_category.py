...
def create_schema_category(schema_category_create: SchemaCategoryCreate, db=Depends(get_db)) -> SchemaCategoryOut:
    return s_create_schema_category(schema_category_create, db=db)
...
def get_schema_category(uuid: str, db=Depends(get_db)) -> SchemaCategoryOut:
    return s_get_schema_category(uuid, db=db)
...
def delete_schema_category(uuid: str, db=Depends(get_db)) -> Message:
    return s_delete_schema_category(uuid, db=db)
...
def update_schema_category(uuid: UUID, schema_category_update: SchemaCategoryUpdate, db=Depends(get_db)) -> SchemaCategoryOut:
    return s_update_schema_category(uuid, schema_category_update=schema_category_update, db=db)
