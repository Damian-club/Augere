...
def r_create_schema_entry(schema_entry_create: SchemaEntryCreate, db: Session = Depends(get_db)) -> SchemaEntryOut:
    return create_schema_entry(schema_entry_create, db=db)


@router.get("/{uuid}", response_model=SchemaEntryOut)
def r_get_schema_entry(uuid: UUID, db: Session = Depends(get_db)) -> SchemaEntryOut:
    return get_schema_entry(uuid, db=db)


@router.put("/{uuid}", response_model=SchemaEntryOut)
def r_update_schema_entry(uuid: UUID, schema_entry_update: SchemaEntryUpdate, db: Session = Depends(get_db)) -> SchemaEntryOut:
    return update_schema_entry(uuid, schema_entry_update=schema_entry_update, db=db)


@router.delete("/{uuid}", response_model=Message)
def r_delete_schema_entry(uuid: UUID, db: Session = Depends(get_db)) -> Message:
